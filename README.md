# db_api
# Deutsche Bahn API Client - Station Klasse

## Beschreibung

Die Klasse `Station` dient als Schnittstelle zur Deutschen Bahn API, um Fahrplan- und Echtzeitdaten für einen bestimmten Bahnhof abzurufen. Sie kapselt die Logik für die Authentifizierung, das Abrufen von Daten und die Verarbeitung der Antworten.
...
* `__str__(self)`
    * **Rückgabewert:**
        * (str): Eine JSON-String-Repräsentation der Stationsdaten (formatiert mit Einrückungen und UTF-8-Unterstützung).
    * **Funktionalität:**
...
1.  Importiere die Klasse `Station`:
    ```python
    from station import Station
    ```

2.  Erstelle eine Instanz der Klasse `Station`:
    ```python
    bahnhof = Station("Dein Bahnhofsname")
    ```

3.  Rufe die gewünschten Daten ab:
    ```python
